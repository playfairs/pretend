...
use crate::config::{load_config, save_config, PretendConfig};
use crate::database::{AppDatabase, CacheStore};
use crate::error::Result;
use crate::models::{App, Presence};
use crate::rpc::{Activity, RpcClient};
...
impl PretendService {
    ...
    pub fn start(&self, application: &str, details: Option<String>, state: Option<String>) -> Result<Presence> {
    ...
    pub fn config_summary(&self) -> String {
        ...
        lines.push(format\!("default_application: {}", self.config.default_application.as_deref().unwrap_or("none")));
        lines.push(format\!("last_application_id: {}", self.config.last_application_id.as_deref().unwrap_or("none")));
        lines.join("
")
    }
}
